...
def train_and_evaluate_semi_supervised_svm(X_labeled, y_labeled, X_unlabeled, X_test, y_test, i, perc):
    print(f"running test iteration {i} with percentage {perc}")

    if len(X_unlabeled) == 0:  # If X_unlabeled is empty, train a regular SVM
        svm_classifier = SVC(probability=True)
        svm_classifier.fit(X_labeled, y_labeled)
        y_pred = svm_classifier.predict(X_test)
    else:
        # Initialize the SVM classifier with probability estimates
        svm_classifier = SVC(probability=True)

        # Create a SelfTrainingClassifier with the SVM
        self_training_model = SelfTrainingClassifier(svm_classifier)
...
def main_optimized(avg_per_perc):
    ...
    for perc in percentages:
        ...
        accuracies = Parallel(n_jobs=3)(delayed(train_and_evaluate_semi_supervised_svm)(
...
if __name__ == "__main__":
    ...
    base_file_name = 'SemiSupervisedSVM1000AVG'
